        ...
        try:
            ...
            if mime_type == 'application/vnd.google-apps.shortcut':
                ...
                if target_id:
                    ...
                    target_file = service.files().get(fileId=target_id, fields="id, name, mimeType, shortcutDetails").execute()
            ...
            if mime_type in exportable_types:
                ...
                while not done:
                    ...
                    logger.info(f"Exporting {file_name} as PDF: {int(status.progress() * 100)}%")
                return
            # If it's a non-downloadable Google file (Forms, Maps, etc.), skip
            if mime_type.startswith('application/vnd.google-apps.'):
                logger.warning(f"Skipping non-downloadable Google Drive file: {file_name} (type: {mime_type})")
                return
...
def download_gdrive_folder(folder_url, output_dir):
    ...
    try:
        ...
        for subfolder_id in subfolders:
            ...
            subfolder_files = [f for f in subfolder_list if f['mimeType'] != 'application/vnd.google-apps.folder']
...
def main():
    ...
    # Setup logging
    log_path = os.path.join(base_dir, f'epstein_downloader_{__import__("datetime").datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
            ...
            if url.startswith('https://drive.google.com/drive/folders/'):
                ...
                if match:
                    ...
                    if os.path.exists(credentials_path):
                        try:
                            download_drive_folder_api(folder_id, gdrive_dir, credentials_path)
                        except Exception as e:
                        ...
                        logger.warning("credentials.json not found. Attempting gdown fallback...")
                        try:
                            import gdown
                            gdown.download_folder(url=url, output=gdrive_dir, quiet=False, use_cookies=False, remaining_ok=True)
                        except Exception as e2:
        ...
        for url, local_path in missing_files:
            ...
            try:
                ...
                if url.startswith('gdrive://'):
                    ...
                    logger.error(f"Cannot redownload missing Google Drive file automatically: {url}")
                ...
                logger.error(f"Failed to download missing file {url}: {e}\nContinuing...")
