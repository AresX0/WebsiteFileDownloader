...
for fn in FILES:
    ...
    report['assets'].append({
        ...
        'preview_path': os.path.relpath(preview_name, ROOT) if preview_saved else None,
    })
...
for a in report['assets']:
    if 'error' in a:
        ...
        print(f"- {a['file']}: {a['width']}x{a['height']}, {pct_trans:.1f}% transparent, {pct_dark:.1f}% dark-by-luminance, preview={'yes' if a['preview_saved'] else 'no'})")
